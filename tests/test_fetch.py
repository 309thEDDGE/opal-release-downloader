...
class TestFetch:
    def test_prepare_local_path(self, mock_os_path, mock_os_makedirs):
        ...
        mock_os_path.join.assert_has_calls(
            [call(join_retval1), call(dest, join_retval1)]
        )
    ...
    def test_prepare_local_path_unlink(
        self, mock_os_path, mock_os_makedirs, mock_os_unlink
    ):
        s3_item = {"Key": "2022.09.07/blind", "Size": 200}
        dest = "/home/user/artifacts"
    ...
    def test_s3_download_with_progress(self, mock_tqdm, mock_os_path):
        ...
        s3_download_with_progress(mock_s3_client, bucket_name, s3_item, local_name)

        mock_os_path.basename.assert_called_once_with(local_name)
        mock_tqdm.assert_called_once_with(
            total=size, unit="B", unit_scale=True, desc="a.b"
        )
    ...
    def test_get_files_path_spec_and_dest_is_none(
        self,
        mock_get_latest,
        mock_list_objects,
        mock_gets3,
        mock_prepare_local_path,
        mock_s3_download,
        mock_os_path,
    ):
        ...
        get_files(bucket_name, path_spec, region_name=region_name, dest=None)

        mock_get_latest.assert_called_once_with(bucket_name, region_name=region_name)
        ...
        mock_list_objects.assert_called_once_with(
            bucket_name, prefix=get_latest_retval, region_name=region_name
        )
    ...
    def test_get_files(
        ...
        get_files(bucket_name, path_spec, region_name=region_name, dest=dest)
    ...
    @patch("opal_release_downloader.fetch.get_s3_client")
    @patch("opal_release_downloader.fetch.list_bucket_objects")
    def test_get_files_no_overwrite(
        self,
        mock_list_objects,
        mock_gets3,
        mock_prepare_local_path,
