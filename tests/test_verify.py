...
class TestVerify:
    ...
    @patch("tqdm.tqdm")
    @patch("hashlib.md5")
    def test_md5sum(self, mock_hashlib, mock_tqdm, mock_open, mock_os_path):
        ...
        read_bytes = [bytes(4096), bytes(4096), bytes(421), bytes(0)]
        mock_f = Mock()
        ...
        assert mock_f.read.mock_calls == [
            call(4096),
            call(4096),
            call(4096),
            call(4096),
        ]
        assert hash.update.mock_calls == [
            call(read_bytes[0]),
            call(read_bytes[1]),
            call(read_bytes[2]),
        ]
        assert mock_tq.update.mock_calls == [call(4096), call(4096), call(421)]
    ...
    def test_read_checksums_from_file(self, mock_open, mock_os_path):
        ...
        assert mock_os_path.basename.mock_calls == [
            call("/home/user/f1"),
            call("/tmp/data/another_one.pq"),
        ]
        assert test_sums == sums

    @patch("builtins.open")
    def test_read_checksums_from_file_malformed(self, mock_open, mock_os_path):
        checksum = "/tmp/hashes/check.md5sum"
        read_lines = [
            "erg8hqw3pt8y /home/user/f1",
    ...
    def test_operate_on_files_fail_if_subdirs(
        self, mock_os_walk, mock_os_getcwd, mock_os_path
    ):
        walk_root = "."
        walk_dirs = ["data", "test"]
        ...
        with pytest.raises(Exception) as e:
            operate_on_files(root_dir, dummy_func, fail_if_subdirs=True)
    ...
    def test_operate_on_files_apply_func(self, mock_os_walk):
        ...
        operate_on_files(root_dir, dummy_func, fail_if_subdirs=False)
    ...
    def test_operate_on_files_excluded_files(self, mock_os_walk):
        ...
        operate_on_files(
            root_dir, dummy_func, fail_if_subdirs=False, excluded_files=excluded_files
        )
    ...
    def test_check_checksums(
        self,
        mock_read_checksums_from_file,
        mock_check_checksums_operator,
        mock_operate_on_files,
    ):
        ...
        check_checksums(checksum, excluded_files=excluded_files, strict=strict)
        ...
        mock_operate_on_files.assert_called_once_with(
            ".",
            operator,
            fail_if_subdirs=fail_if_subdirs,
    ...
    def test_check_manifest(
        ...
        mock_yaml.load.assert_called_once_with(mock_f, Loader=mock_yaml.SafeLoader)
        mock_tqdm.tqdm.assert_called_once_with(
            desc="checking_manifest", total=len(expected_files)
        )
        mock_operate_on_files.assert_called_once_with(
            ".",
            operator,
    ...
    @patch("builtins.open")
    @patch("opal_release_downloader.verify.tqdm")
    def test_check_manifest_file_not_found(
        self,
        mock_tqdm,
        mock_open,
        mock_yaml,
    ...
    def test_find_file_and_confirm_glob_search_fail(self, mock_glob):
        glob_str = "a.*"
    ...
    @patch("opal_release_downloader.verify.glob")
    def test_find_file_and_confirm_glob_search_succeed(self, mock_glob, mock_os_path):
        glob_str = "a.*"
    ...
    @patch("opal_release_downloader.verify.glob")
    def test_find_file_and_confirm_glob_result_not_exist(self, mock_glob, mock_os_path):
        glob_str = "a.*"
        glob_search_result = ["a.txt"]
        mock_glob.glob.return_value = glob_search_result
    ...
    def test_find_file_and_confirm_no_search_file_not_exist(self, mock_os_path):
        glob_str = "a.*"
        file_name = "a.txt"
        mock_os_path.exists.return_value = False
        with pytest.raises(Exception) as e:
            find_file_and_confirm(glob_str, file_name=file_name, search=False)
        mock_os_path.exists.assert_called_once_with(file_name)

    def test_find_file_and_confirm_no_search_file_exist(self, mock_os_path):
        glob_str = "a.*"
        file_name = "a.txt"
        mock_os_path.exists.return_value = True
        result = find_file_and_confirm(glob_str, file_name=file_name, search=False)
        mock_os_path.exists.assert_called_once_with(file_name)
        assert result == file_name

    def test_verify_directory_dir_not_exist(self, mock_os_getcwd, mock_os_path):
    ...
    def test_verify_directory_not_dir(self, mock_os_getcwd, mock_os_path):
        directory = "/home/Sprocket/files"
        checksum = "checksum.txt"
        manifest = "manifest.txt"
        search = True
    ...
    def test_verify_directory_require_manifest(
        self,
        mock_find_file,
        mock_check_manifest,
        mock_check_checksums,
        mock_os_getcwd,
        mock_os_path,
        mock_os_chdir,
    ):
        ...
        mock_check_manifest.assert_called_once_with(manifest, excluded_files=[checksum])
        mock_check_checksums(
            checksum, excluded_files=[checksum], strict=strict_checksum
        )

    @patch("opal_release_downloader.verify.check_checksums")
    @patch("opal_release_downloader.verify.find_file_and_confirm")
    def test_verify_directory_no_require_manifest(
        self,
        mock_find_file,
        mock_check_checksums,
        mock_os_getcwd,
        ...
        mock_find_file.assert_called_once_with(
            "md5sums_*", file_name=checksum, search=search
        )
        mock_check_checksums(
            checksum, excluded_files=[checksum], strict=strict_checksum
        )
